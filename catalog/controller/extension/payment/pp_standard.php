...
class ControllerExtensionPaymentPPStandard extends Controller {
	public function index() {
		...
		if ($order_info) {
			...
			$data['invoice'] = $this->session->data['order_id'] . ' - ' . html_entity_decode($order_info['payment_fullname'], ENT_QUOTES, 'UTF-8') . ' ' . html_entity_decode($order_info['payment_lastname'], ENT_QUOTES, 'UTF-8');
