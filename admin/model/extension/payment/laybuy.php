...
class ModelExtensionPaymentLaybuy extends Model {
	...
	public function getTransactions($data = array()) {
		$sql = "SELECT *, CONCAT(fullname, ' ', lastname) AS `customer` FROM `" . DB_PREFIX . "laybuy_transaction` `lt` WHERE 1 = 1";
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "CONCAT(fullname, ' ', lastname) LIKE '%" . $this->db->escape($data['filter_customer']) . "%'";
		}
