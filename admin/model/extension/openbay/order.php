...
class ModelExtensionOpenBayOrder extends Model {
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND CONCAT(`fullname`, ' ', `lastname`) LIKE '%" . $this->db->escape($data['filter_customer']) . "%'";
		}
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND LCASE(CONCAT(o.fullname, ' ', o.lastname)) LIKE '" . $this->db->escape(utf8_strtolower($data['filter_customer'])) . "%'";
		}
